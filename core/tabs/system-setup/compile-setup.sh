...
installDepend() {
    ...
    DEPENDENCIES='tar tree multitail trash-cli unzip cmake make jq pkg-config git python3 go'
    ...
    case "$PACKAGER" in
        pacman)
            ...
            ;;
        apt-get|nala)
            COMPILEDEPS='build-essential python3-pip golang'
        ...
        dnf)
            ...
            "$ESCALATION_TOOL" "$PACKAGER" -y install $DEPENDENCIES python3-pip golang
        ...
        zypper)
            ...
            "$ESCALATION_TOOL" "$PACKAGER" --non-interactive install tar tree multitail unzip cmake make jq pkg-config git python3 python3-pip go libgcc_s1-gcc7-32bit glibc-devel-32bit
            ;;
        apk)
            "$ESCALATION_TOOL" "$PACKAGER" add build-base multitail tar tree trash-cli unzip cmake jq pkg-config git python3 py3-pip
            ;;
        xbps-install)
            COMPILEDEPS='base-devel python3-pip go'
        ...
        eopkg)
            ...
            "$ESCALATION_TOOL" "$PACKAGER" install -y tar tree unzip cmake make jq pkg-config git python3 golang
