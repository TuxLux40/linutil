...
configure_kde_shortcuts() {
    printf "%b\n" "${YELLOW}Configuring KDE keyboard shortcuts...${RC}"
    
    # Backup existing shortcuts
    if [ -f "$HOME/.config/kglobalshortcutsrc" ]; then
        cp "$HOME/.config/kglobalshortcutsrc" "$HOME/.config/kglobalshortcutsrc.backup.$(date +%Y%m%d_%H%M%S)"
        printf "%b\n" "${GREEN}Backed up existing shortcuts.${RC}"
    fi
    
    mkdir -p "$HOME/.config"
    
    # Create the shortcuts configuration file
    cat > "$HOME/.config/kglobalshortcutsrc" << 'EOF'
[ActivityManager]
_k_friendly_name=Activity Manager
switch-to-activity-2518ebcb-a145-4bc4-a2b9-397b9a4bb4c0=none,none,Switch to activity "Default"
...
activate widget 28=none,none,Activate System Tray Widget
activate widget 54=none,none,Activate Application Menu Widget
...
[services][com.brave.Browser.beta.desktop]
_launch=Meta+B

[services][com.mitchellh.ghostty.desktop]
_launch=Meta+X

[services][com.rtosta.zapzap.desktop]
_launch=Meta+Alt+M

[services][com.yubico.yubioath.desktop]
_launch=Meta+Alt+P

[services][org.kde.dolphin.desktop]
_launch=Meta+E\tMeta+F

[services][org.kde.krunner.desktop]
_launch=Search\tAlt+F2\tAlt+Space\tMeta

[services][org.kde.kscreen.desktop]
ShowOSD=Display

[services][proton-mail.desktop]
_launch=Launch Mail

[services][proton-pass.desktop]
_launch=Meta+P

[services][signal-desktop.desktop]
_launch=Meta+M

[services][systemsettings.desktop]
_launch=Meta+S\tTools
...
EOF
    ...
    printf "%b\n" "${GREEN}KDE shortcuts configured successfully.${RC}"
}
...
show_important_shortcuts() {
    printf "%b\n" "${YELLOW}=== Important Custom Shortcuts ===${RC}"
    printf "%b\n" "${CYAN}Window Management:${RC}"
    printf "  Meta+Q          - Kill Window\n"
    printf "  Meta+W          - Overview\n"
    printf "  Meta+T          - Toggle Tiles Editor\n"
    printf "  Meta+G          - Grid View\n"
    printf "  Meta+D          - Show Desktop\n"
    printf "  Meta+Arrow Keys - Quick Tile Window\n"
    printf "  Meta+PgUp/PgDn  - Maximize/Minimize\n"
    printf "\n"
    printf "%b\n" "${CYAN}Desktop Navigation:${RC}"
    printf "  Ctrl+F1-F4      - Switch to Desktop 1-4\n"
    printf "  Meta+Ctrl+Arrow - Switch Desktop\n"
    printf "  Meta+Tab        - Walk Through Windows\n"
    printf "\n"
    printf "%b\n" "${CYAN}Application Launchers:${RC}"
    printf "  Meta            - KRunner\n"
    printf "  Meta+B          - Brave Browser Beta\n"
    printf "  Meta+X          - Ghostty Terminal\n"
    printf "  Meta+E/F        - Dolphin File Manager\n"
    printf "  Meta+P          - Proton Pass\n"
    printf "  Meta+M          - Signal Desktop\n"
    printf "  Meta+S          - System Settings\n"
    printf "  F12             - Yakuake Terminal\n"
    printf "\n"
    printf "%b\n" "${CYAN}Session:${RC}"
    printf "  Meta+L          - Lock Session\n"
    printf "  Ctrl+Alt+Del    - Logout Screen\n"
    printf "\n"
}
...
printf "%b\n" "${GREEN}All KDE shortcuts have been configured!${RC}"
printf "%b\n" "${YELLOW}Note: You may need to log out and back in for all changes to take effect.${RC}"
